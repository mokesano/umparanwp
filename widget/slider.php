<?php
function sliderViewDesktop($category,$jml,$titletrending,$amounttrending,$rentang){
	global $post;
...
function sliderViewMobile($category,$jml){
	global $post;
	if(!empty($category) && $category != ""):
		$category_link = get_category_link( get_cat_ID( $category ) );
		$args = array(
...
class sliderHome extends WP_Widget {

	public function __construct() {
		$idwidget = 'sliderhome';
		$namewidget = '✔️ Slider Home';
	...
	public function widget( $args, $instance ) {
		if($instance['mobile'] == 'yes'){
			if (function_exists( 'is_amp_endpoint' ) && is_amp_endpoint()) {
				sliderViewMobile($instance['headline'], $instance['amountheadlinemobile']);
			}elseif (wp_is_mobile()) {
				sliderViewMobile($instance['headline'], $instance['amountheadlinemobile']);
			}
		}
		if($instance['desktop'] == 'yes'){
			if (function_exists( 'is_amp_endpoint' ) && is_amp_endpoint()) {
				...
				sliderViewDesktop($instance['headline'], $instance['amountheadlinedesktop'], $instance['titletrending'], $instance['amounttrending'], $instance['rentang']);
			}
		}
	}
...
function sliderHomeload() {
	register_widget( 'sliderHome' );
}
add_action( 'widgets_init', 'sliderHomeload' );