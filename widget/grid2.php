<?php
function grid2ViewMobile($judul,$category,$jml){
	global $post;
...
function grid2ViewDesktop($judul,$category,$jml){
	global $post;
	if(!empty($category) && $category != ""):
		$category_link = get_category_link( get_cat_ID( $category ) );
		if(is_single()):
...
class grid2Widget extends WP_Widget {
	public function __construct() {
		$idwidget = 'grid2';
		$namewidget = '✔️ Grid2';
		$descwidget = 'Widget ini menampilan daftar pos berdasarkan kategori yang dipilih dan ditampilkan dalam bentuk widget grid2';
		parent::__construct($idwidget, $namewidget, array('description'=> $descwidget));
	}
	public function widget( $args, $instance ) {
		if($instance['mobile'] == 'yes'){
			if (function_exists( 'is_amp_endpoint' ) && is_amp_endpoint()) {
				grid2ViewMobile($instance['title'], $instance['category'], $instance['amountmobile']);
			}elseif (wp_is_mobile()) {
				grid2ViewMobile($instance['title'], $instance['category'], $instance['amountmobile']);
			}
		}
		if($instance['desktop'] == 'yes'){
			if (function_exists( 'is_amp_endpoint' ) && is_amp_endpoint()) {
				...
				grid2ViewDesktop($instance['title'], $instance['category'], $instance['amountdesktop']);
			}
		}
	}
...
function grid2Widgetload() {
	register_widget( 'grid2Widget' );
}
add_action( 'widgets_init', 'grid2Widgetload' );